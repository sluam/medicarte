...
require_once (__DIR__."/../entidad/receta.php");
...
class UsuarioDAO
{
    ...
    { // Login
        ...
        if (count($data_table) == 1) {
            foreach ($data_table as $indice => $valor) {
                $receta = new Receta(
                    $data_table[$indice]["idreceta"],
                    $data_table[$indice]["fecha_expedicion"],
                    $data_table[$indice]["fecha_vencimiento"],
                    $data_table[$indice]["paciente_usuario_cedula"]
    ...
    public function buscarUsuarioPorCedula($cedula)
    {
        ...
        $data_table = $data_source->ejecutarConsulta(
            "SELECT * FROM receta WHERE paciente_usuario_cedula = :cedula",
            array(':cedula' => $cedula)
        );
        $receta = null;
        if (count($data_table) == 1) {
    ...
    {
        $data_source = new DataSource();
        $sql = "INSERT INTO usuario VALUES (:nombre, :apellido,:telefono,:direccion,:cedula,:password,:estado,:edad,:correo,:eps_ideps)";
